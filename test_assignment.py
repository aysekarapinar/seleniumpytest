...
class Test_work():
    ...
    def getData():
        excelFile = openpyxl.load_workbook(globalConstants.invalidLogin)
    ...
    def test_invalid_login(self,username,password):
        ...
        loginButton = self.driver.find_element(By.ID,globalConstants.loginButton_id)
        loginButton.click()
        errorMessage = self.driver.find_element(By.XPATH, globalConstants.errorEmpty)
    ...
    def test_incorrectConditions(self):
        ...
        password.send_keys("")
        logginButton=self.driver.find_element(By.ID,globalConstants.loginButton_id)
        logginButton.click()
        sleep(2)
        errorMassage=self.driver.find_element(By.XPATH,globalConstants.errorEmpty)
        assert errorMassage.text=="Epic sadface: Username is required"


    def getData2():
        excelFile = openpyxl.load_workbook(globalConstants.validLogin) 
    ...
    def test_login(self,username,password):
        userr=self.driver.find_element(By.ID,globalConstants.username_id)
        userr.send_keys(username)
        sleep(2)
        passwo=self.driver.find_element(By.ID,globalConstants.password_id)
        passwo.send_keys(password)
        sleep(3)
        logginButton=self.driver.find_element(By.ID,globalConstants.loginButton_id)
        logginButton.click()
        sleep(3)
        baslik=self.driver.find_element(By.XPATH,globalConstants.app_logo)
        assert baslik.text=="Swag Labs"


    def veritabanini_yukle():
     with open("database/users.json") as dosya:
